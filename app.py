...
def init_firebase():
    if not firebase_admin._apps:
        try:
            ...
            # 2. 강제 타입 보정: 모든 값을 문자열로 변환하고 줄바꿈 수정
            # 이 과정이 딕셔너리 구조를 Firebase가 이해하는 형태로 다듬어줍니다.
            formatted_config = {
                "type": str(config["type"]),
                "project_id": str(config["project_id"]),
                "private_key_id": str(config["private_key_id"]),
                "private_key": str(config["private_key"]).replace("\\n", "\n"),
                "client_email": str(config["client_email"]),
                "client_id": str(config["client_id"]),
                "auth_uri": str(config["auth_uri"]),
                "token_uri": str(config["token_uri"]),
                "auth_provider_x509_cert_url": str(config["auth_provider_x509_cert_url"]),
                "client_x509_cert_url": str(config["client_x509_cert_url"])
            }
            
            # 3. 신뢰할 수 있는 방식으로 Credential 생성
            cred = credentials.Certificate(formatted_config)
